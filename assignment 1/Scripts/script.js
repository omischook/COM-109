...
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.nav-btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
          console.log("Button clicked"); // Debugging to check button press
          ...
          if (content.style.display === 'block') {
              ...
              content.style.display = 'block'; // Show this content
          }
      });
  });
});
...
// local storage and order code goes here ---------------------------------------------------------------------------------'
document.addEventListener('DOMContentLoaded', function() {
  displayCartContents();
});

function displayCartContents() {
  const cartContents = document.getElementById('orderDetailsBox');
  
  // Only proceed if the cartContents element exists on the page
  if (cartContents) {
      const storedItems = JSON.parse(localStorage.getItem('orderedItems')) || [];
      console.log('Stored items from localStorage:', JSON.parse(localStorage.getItem('orderedItems')));


      if (storedItems.length > 0) {
          cartContents.innerHTML = ''; // Clear any default or previous contents
          ...
          cartContents.textContent = 'No items in cart.';
      }
  } else {
      console.log('Cart contents element not found on this page. No action taken.');
  }
}
